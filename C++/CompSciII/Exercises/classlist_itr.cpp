...
//     class and a waiting list.  This is version uses interators with the vector. 
...
void remove_student( const string& id_number, 
                     ...
                     vector<string>& enrolled,
                     vector<string>& waiting );
void erase_from_vector( vector<string>::iterator p,
			vector<string>& v );
...
main()
{
  ...
  for ( vector<string>::iterator e_itr = enrolled.begin(); 
	e_itr != enrolled.end(); ++ e_itr )
    {
      cout << *e_itr << endl;    // *e_itr is the string stored in the vector loc
    }

  if ( ! waiting.empty() )
    {
      cout << "\nStudents are on the waiting list in the following order:\n";
      for ( vector<string>::iterator w_itr = waiting.begin(); 
                ...
                vector<string>& waiting )
{
  ...
  bool found = false;
  vector<string>::iterator loc = enrolled.begin();
...
void
erase_from_vector( vector<string>::iterator p,
                   vector<string>& v )
{
  vector<string>::iterator q = p; 
